...
public class GlobalGraphQLExceptionHandler {
    ...
    public GraphQLError handleException(AbstractNotFoundException ex, DataFetchingEnvironment env) {
        log.warn(ex.getMessage());
        return buildError(ex, env, ErrorType.NOT_FOUND);
    }

    @GraphQlExceptionHandler
    public GraphQLError handleException(AbstractBadRequestException ex, DataFetchingEnvironment env) {
        log.warn(ex.getMessage());
